# ============================================================
# app.py -- Instacart Revenue Protection Dashboard
# ============================================================
import os
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("**GitHub**")
    st.markdown("[instacart-churn-retention-analysis](https://github.com/91Sakthivel/instacart-churn-retention-analysis)")
...
if page == "Executive Dashboard":
    ...
    with col_l:
        ...
        if len(rfm) > 0 and "segment" in rfm.columns:
            ...
            fig = px.bar(seg_cnt, x="Segment", y="Customers",
    ...
    with col_r:
        ...
        if len(traj) > 0 and "trajectory_type" in traj.columns and "churned" in traj.columns:
            tt = (traj.groupby("trajectory_type")
            ...
            fig2 = px.bar(tt, x="label", y="churn_pct", text="churn_pct",
                          color="trajectory_type", color_discrete_map=C_TRAJ)
...
# ============================================================
    ...
    with f1:
        st.markdown("""
        <div style="background:#fff5f5;border-left:4px solid #e74c3c;border-radius:6px;padding:20px;height:100%;">
          ...
          <div style="font-size:0.9rem;color:#444;line-height:1.6;">
            ...
            notifications costing $0.50 each.
          </div>
        </div>""", unsafe_allow_html=True)

    with f2:
        st.markdown("""
        <div style="background:#fff8f0;border-left:4px solid #e65100;border-radius:6px;padding:20px;height:100%;">
          ...
          <div style="font-size:0.9rem;color:#444;line-height:1.6;">
            High-value customers showing gap drift (Type D) have only 22.3% churn rate.
            A loyalty reward at $8.50 cost recovers 40% of them at 17,891% ROI.
          </div>
        </div>""", unsafe_allow_html=True)

    with f3:
        st.markdown("""
        <div style="background:#f0f7ff;border-left:4px solid #2980b9;border-radius:6px;padding:20px;height:100%;">
          ...
          <div style="font-size:0.9rem;color:#444;line-height:1.6;">
            Produce carries 29% of total 60-day revenue exposure — pure volume concentration risk,
            no operational anomaly detected.
          </div>
        </div>""", unsafe_allow_html=True)
    ...
    with a1:
        st.markdown("""
        <div style="background:#f0fff4;border:1px solid #27ae60;border-top:4px solid #27ae60;border-radius:6px;padding:20px;">
          <div style="font-size:0.75rem;font-weight:700;color:#27ae60;text-transform:uppercase;margin-bottom:8px;">Do This Today</div>
          <div style="font-weight:700;font-size:1.05rem;color:#1a1a2e;margin-bottom:10px;">Type D Loyalty Reward Campaign</div>
          <div style="font-size:0.85rem;color:#444;line-height:1.7;">
            <b>Trigger:</b> reorder_rate &ge; 0.55 AND gap_trend &gt; 5<br>
            <b>Customers:</b> 9,786 &nbsp;|&nbsp; <b>Cost:</b> $8.50<br>
            <b>Recovery:</b> 40% &nbsp;|&nbsp; <b>ROI:</b> 17,891%
          </div>
        </div>""", unsafe_allow_html=True)

    with a2:
        st.markdown("""
        <div style="background:#fffbf0;border:1px solid #e65100;border-top:4px solid #e65100;border-radius:6px;padding:20px;">
          <div style="font-size:0.75rem;font-weight:700;color:#e65100;text-transform:uppercase;margin-bottom:8px;">Do This This Week</div>
          <div style="font-weight:700;font-size:1.05rem;color:#1a1a2e;margin-bottom:10px;">Type B Sudden Stop Outreach</div>
          <div style="font-size:0.85rem;color:#444;line-height:1.7;">
            <b>Trigger:</b> early_avg_gap &lt; 12 AND late_avg_gap_clean &gt; 20<br>
            <b>Customers:</b> 6,010 &nbsp;|&nbsp; <b>Cost:</b> $15.00<br>
            <b>Recovery:</b> 20% &nbsp;|&nbsp; <b>ROI:</b> 3,820%
          </div>
        </div>""", unsafe_allow_html=True)

    with a3:
        st.markdown("""
        <div style="background:#fff5f5;border:1px solid #e74c3c;border-top:4px solid #e74c3c;border-radius:6px;padding:20px;">
          <div style="font-size:0.75rem;font-weight:700;color:#e74c3c;text-transform:uppercase;margin-bottom:8px;">Stop Doing This</div>
          <div style="font-weight:700;font-size:1.05rem;color:#1a1a2e;margin-bottom:10px;">Type C Blanket Discounting</div>
          <div style="font-size:0.85rem;color:#444;line-height:1.7;">
            <b>At-risk customers:</b> 89,144<br>
            <b>Annual waste:</b> $980,584 if sent $11 discount<br>
            <b>Correct offer:</b> $0.50 push notification only
          </div>
        </div>""", unsafe_allow_html=True)
    ...
    st.markdown("""
    <div style="background:#f8f9fa;border:1px solid #dee2e6;border-radius:6px;padding:20px;font-size:0.92rem;color:#444;line-height:1.8;">
      ...
      regressor has R&#178;=0.18 — revenue loss per customer is a directional estimate,
      not a precise forecast. Retrain monthly on fresh order data.
    </div>""", unsafe_allow_html=True)
...
# ============================================================
    ...
    st.download_button("Download filtered results as CSV", csv_bytes,
                       "at_risk_customers_filtered.csv", "text/csv")

# ============================================================
# PAGE 4 — Intervention & ROI
# ============================================================
    ...
    if len(roi) > 0:
        ...
        fig = px.bar(rc, x="label", y="avg_ROI_pct", text="avg_ROI_pct",
                     color="trajectory_type", color_discrete_map=C_TRAJ)
...
# ============================================================
    ...
    if len(cat_risk) > 0 and "dept_total_risk" in cat_risk.columns:
        ...
        fig = px.bar(cat_sorted, x="dept_total_risk", y="department",
        ...
        st.info("category_risk_report.csv not available.")
    ...
    if len(dept_res) > 0:
        ...
        dr = dr.rename(columns={"primary_department": "Department",
                                 "customer_count": "Customers", "status": "Status"})
...
# ============================================================
    ...
    feat_df = pd.DataFrame({
        "Feature": ["avg_days_between_orders", "order_gap_std", "order_gap_cv",
                    "early_avg_gap", "late_avg_gap_clean", "order_count", "gap_trend", "reorder_rate"],
        "Description": [
    ...
    LEAKS = [
        ...
        ("Round 3", "Rebuilt Type B on pure gap signals — redefined as early_avg_gap < 12 AND late_avg_gap_clean > 20"),
    ]
    for rnd, line in LEAKS:
        st.markdown(f"""
        <div class="leak-row"><b>{rnd}:</b> {line}</div>""", unsafe_allow_html=True)
    ...
    rr1.metric("Regressor RMSE", "$23.98")
    rr2.metric("Pareto (80% of risk)", "Top 93,043 customers")

    st.markdown("---")
    st.subheader("Feature Importance — Churn Classifier")
    fi_path = CHART_FEAT_IMP
    if fi_path.exists():
        st.image(str(fi_path), width=700)
    else:
